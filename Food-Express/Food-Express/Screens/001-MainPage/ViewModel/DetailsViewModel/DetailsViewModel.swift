...
extension DetailsViewModel: DetailsViewModelInterface {
            ...
            case .success(let data):
                if data.success == 0 {
                    self.view?.showAlert(status: false,
                                         title: "Failed",
                                         message: "Your product could not be added to the basket due to an unknown reason.")
    ...
    func addToFavorite(newFavorite: Favorites) {
        APICaller.updateUserFavorites(newFavorite: newFavorite) { [ weak self ] error in
            ...
            if let error = error {
                self.view?.showAlert(status: false,
                                     title: "Failed",
                                     message: "Something went wrong while adding favorites.")
    ...
    func isFavorite(foodName: String) {
        APICaller.fetchUserFavorites { [ weak self ] favorites, error in
            ...
            isLiked = self.isFavoriteOrNot(foodName: foodName,
                                           favorites: favorites)
